...
namespace MSDN.Samples.ClaimsAuth
{
    public static class ClaimClientContext
    {
        ...
        /// <param name="targetSiteUrl"></param>
        /// <returns></returns>
        public static void GetAuthenticatedContext<T>(T context) where T : ClientContext
        {
            GetAuthenticatedContext(context, 0, 0);
        }
        ...
        /// <param name="targetSiteUrl"></param>
        /// <returns></returns>
        public static T GetAuthenticatedContext<T>(string targetSiteUrl) where T : ClientContext
        {
            T context = (T)Activator.CreateInstance(typeof(T), targetSiteUrl);
            return (GetAuthenticatedContext<T>(context, 0, 0));
        }
        /// <summary>
        ...
        /// <param name="targetSiteUrl"></param>
        /// <returns></returns>
        public static T GetAuthenticatedContext<T>(T context, int popUpWidth, int popUpHeight) where T : ClientContext
        {
            CookieCollection cookies = null;
            cookies = ClaimClientContext.GetAuthenticatedCookies(context.Url, popUpWidth, popUpHeight);
        ...
        public static ClientContext GetAuthenticatedContext(string targetSiteUrl, int popUpWidth, int popUpHeight)
        {
            ...
            GetAuthenticatedContext(context, popUpWidth, popUpHeight);
            return context;
        }
    }
}
